          ...
          {ZONES.map((zone) => (
            <button
              ...
              className={`px-4 py-2 rounded text-white transition-colors ${
                selectedZone === zone ? "bg-blue-700" : "bg-blue-500 hover:bg-blue-600"
              }`}
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {sights.map((s, i) => (
              <div
                key={i}
                className="w-full bg-white rounded-xl shadow-lg overflow-hidden transition-transform hover:scale-[1.02] duration-300 h-fit"
              >
                <div className="aspect-w-16 aspect-h-9">
                  <img src={s.photoURL} alt={s.sightName} className="w-full h-48 object-cover" />
                </div>
                <div className="p-4">
          ...
          {selectedSight && (
            <div className="p-6">
              ...
              <div className="text-gray-600 mb-2">
                <span className="font-semibold">區域：</span>
                {selectedSight.zone}
              </div>
              <div className="text-gray-600 mb-2">
              ...
              <div className="text-gray-700 whitespace-pre-line">{selectedSight.description}</div>
            </div>
          )}
        </Modal>
      </div>
